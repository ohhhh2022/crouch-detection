...
import base64
from TCP_Send import tcp_send, get_host_ip
...
class VideoReader(object):
    ...
    def __iter__(self):
        self.cap = cv2.VideoCapture(self.file_name, cv2.CAP_GSTREAMER)
...
def run_demo(net, image_provider, height_size, cpu, track, smooth, json_file):
    net = net.eval()
    if not cpu:
        net = net.cuda()
    ...
    for img in image_provider:
        ...
        heatmaps, pafs, scale, pad = infer_fast(net, img, height_size, stride, upsample_ratio, cpu)   # 1e-2
        ...
        pose_entries, all_keypoints = group_keypoints(all_keypoints_by_type, pafs)   # 1e-3
        # 1e-5
        ...
        for pose in current_poses:
            pose.draw(img)  # 绘制关键点
        # 1e-7
        img = cv2.addWeighted(orig_img, 0.6, img, 0.4, 0)   # 1e-3
        # 1e-6
        for pose in current_poses:
            ...
            if count*args.step > 360 or count_time*args.step >360:
                # 当检测出下蹲动作时，记录时间和事件
                squat_event = {"IP": get_host_ip(),
                                "Time": str(datetime.now()),
                                "Motion": 'crouch'}
                squat_events.append(squat_event)
                
                tcp_send(host, port, squat_event)
                

                cv2.putText(img, 'motion: {}'.format('crouch'), (pose.bbox[0], pose.bbox[1] - 16),
                            cv2.FONT_HERSHEY_COMPLEX, 0.5, (0, 0, 255))
        # 1e-6
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cpu', default=False,help='run network inference on cpu')
    parser.add_argument('--track', action='store_true',default=True, help='track pose id in video')
    parser.add_argument('--smooth', type=int, default=1, help='smooth pose keypoints')
    parser.add_argument('--Squat', action='store_true', default=True, help='Squat detection')
    parser.add_argument('--dis_thres', type=int, default=160, help='keypoint distance')
    parser.add_argument('--min_angle_thres', type=float, default=30., help='keypoint min angle thres')
    parser.add_argument('--max_angle_thres', type=float, default=135., help='keypoint max angle thres')
    parser.add_argument('--step', type=int, default=180, help='power step')
    ...
    if args.video != '':
        ...
        #     size = frame.shape
        #     cv2.imshow('Lightweight Human Pose Estimation Python Demo', frame)
        #     if cv2.waitKey(1) in [ord('q'), 27]:
        #         break
        # fourcc = cv2.VideoWriter_fourcc(*'XVID')
        # out = cv2.VideoWriter("test.avi", fourcc, 120, (size[1], size[0]))

    else:
        args.track = 0
    run_demo(net, frame_provider, args.height_size, args.cpu, args.track, args.smooth, args.json_file)
